...
// Método para verificar si todas las pilas tienen el mismo color
private void checkIfWon() {
    boolean hasWon = true;

    // Recorre cada pila en el array de viales
    for (int i = 0; i < viales.length; i++) {
        Stack<String> vial = viales[i];

        // Ignora los viales vacíos
        if (vial.isEmpty()) continue;

        // Toma el primer color de la pila como referencia
        String firstColor = vial.peek();

        // Recorre la pila con un bucle tradicional, usando tu implementación personalizada
        for (int j = 0; j <= vial.tope; j++) {
            String color = vial.getColorAt(j);
            if (!color.equals(firstColor)) {
                hasWon = false; // Si algún color no coincide, el jugador no ha ganado
                break;
            }
        }

        if (!hasWon) break; // Si encontramos una pila que no es homogénea, salimos del bucle
    }

    // Si todas las pilas tienen colores homogéneos, muestra el cuadro de diálogo de victoria
    if (hasWon) {
